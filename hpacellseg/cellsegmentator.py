...
class CellSegmentator(object):
    """Uses pretrained DPN-Unet models to segment cells from input."""
    ...
    def __init__(
        ...
        """Class for segmenting nuclei and whole cells from confocal microscopy input.

        It takes lists of input and returns the raw output from the
        specified segmentation model. Models can be automatically
        downloaded if they are not already available on the system.

        When working with input from the Huan Protein Cell atlas, the
        ...
        available. One that works with 2 channeled input and one that
        takes 3 channels.
        ...
        scale_factor -- How much to scale input before they are fed to
        ...
        padding -- Whether to add padding to the input before feeding the
                   input to the network. (default: False).
    ...
    def _image_conversion(self, images):
        """Convert/Format input to RGB image arrays list for cell predictions.

        Intended for internal use only.

        Keyword arguments:
        input -- list of lists of image paths/arrays. It should following the
        ...
        if not isinstance(microtubule_imgs, list):
            raise ValueError("The microtubule input should be a list")
        if not isinstance(nuclei_imgs, list):
            raise ValueError("The microtubule input should be a list")

        if er_imgs:
            if not len(microtubule_imgs) == len(er_imgs) == len(nuclei_imgs):
                raise ValueError("The lists of input needs to be the same length")
        else:
    ...
    def pred_nuclei(self, images):
        ...
        Keyword arguments:
        input -- A list of image arrays or a list of paths to input.
                  If as a list of image arrays, the input could be 2d input
                  of nuclei data array only, or must have the nuclei data in
                  the blue channel; If as a list of file paths, the input
    ...
    def pred_cells(self, images, precombined=False):
        """Predict the cell segmentation for a list of input.
        ...
        input -- list of lists of image paths/arrays. It should following the
                  ...
                  The input needs to be of the same size.
        precombined -- If precombined is True, the list of input is instead supposed to be
