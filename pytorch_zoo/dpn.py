...
def dpn68(target_channel_num=1000, pretrained='imagenet'):
    model = DPN(
        small=True, num_init_features=10, k_r=128, groups=32,
        k_sec=(3, 4, 12, 3), inc_sec=(16, 32, 32, 64),
        target_channel_num=target_channel_num, test_time_pool=True)
    if pretrained:
        settings = pretrained_settings['dpn68'][pretrained]
        assert target_channel_num == settings['target_channel_num'], \
            "target_channel_num should be {}, but is {}".format(settings['target_channel_num'], target_channel_num)
...
def dpn68b(target_channel_num=1000, pretrained='imagenet+5k'):
    model = DPN(
        small=True, num_init_features=10, k_r=128, groups=32,
...
def dpn92(in_channels, target_channel_num=1000, pretrained='imagenet+5k'):
    model = DPN(
...
def dpn98(target_channel_num=1000, pretrained='imagenet'):
    model = DPN(
...
def dpn131(target_channel_num=1000, pretrained='imagenet'):
    model = DPN(
...
def dpn107(target_channel_num=1000, pretrained='imagenet+5k'):
    model = DPN(
...
class DPN(nn.Module):
    def __init__(self, small=False, num_init_features=64, k_r=96, groups=32,
                 ...
                 target_channel_num=1000, test_time_pool=False):
        ...
        self.classifier = nn.Conv2d(in_chs, target_channel_num, kernel_size=1, bias=True)
